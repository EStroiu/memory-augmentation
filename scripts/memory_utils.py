...
# ---------- Token budgeting & adaptive compression ----------
...
def assemble_prompt_budgeted(
    ...
    parts.append(f"Target: GAME {target.game_id} | QUEST {target.quest}")
    parts.append("")
    # We'll add target context only if needed (later) because original design put only memory entries.
    parts.append("Memory context (top-k similar rounds):")
    base_prefix = "\n".join(parts) + "\n"
    current = base_prefix
    ...
    for i, (entry, score) in enumerate(retrieved, start=1):
        ...
        meta["truncated"] = True
        break
    # Add task instruction and Answer tag
    ...
    if est_final > max_tokens:
        meta["truncated"] = True
        # Append target minimal context to assist reasoning (was absent beyond ID). Provide compressed target summary.
        target_header = _entry_header_only(target.text)
        target_compressed = _compress_entry_text(target.text, max_transcript_lines=10)
        # Try adding compressed target summary at end (preface)
